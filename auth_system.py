...
def is_moderator(db, user_id: str) -> bool:
    """
    Return True if the user holds verified moderator privileges.

    The isModerator flag is set ONLY via direct database access — there is no
    API endpoint that can grant it. This keeps the privilege-escalation surface
    at zero: even a compromised JWT cannot produce a moderator.
    """
    if not user_id:
        return False
    user = db.users.find_one({"userId": user_id}, {"isModerator": 1})
    return bool(user and user.get("isModerator", False))
